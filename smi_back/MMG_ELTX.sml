class: STATUS_MMG_ELTXTOP_ATLAS_STATUS_CLASS
!panel: ATLAS_STATUS.pnl
    state: UNINITIALIZED	!color: _3DFace
        when ( STATUS_MMG_ELTXATLAS_STATUS_FWSETACTIONS empty ) stay_in_state
        
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state {UNINITIALIZED,DEAD} ) stay_in_state
        
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state FATAL ) move_to FATAL
        
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state ERROR ) move_to ERROR
        
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state WARNING ) move_to WARNING
        
when ( all_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state OK ) move_to OK
        

    state: OK	!color: FwStateOKPhysics
        when ( STATUS_MMG_ELTXATLAS_STATUS_FWSETACTIONS empty ) move_to UNINITIALIZED
        
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state {UNINITIALIZED,DEAD} ) move_to UNINITIALIZED
...
when ( all_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state OK ) stay_in_state
...
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state WARNING ) stay_in_state
...
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state ERROR ) stay_in_state
...
when ( any_in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES in_state FATAL ) stay_in_state
...
object: STATUS_MMG_ELTX is_of_class STATUS_MMG_ELTXTOP_ATLAS_STATUS_CLASS

class: STATUS_MMG_ELTXATLAS_STATUS_FwDevMode_CLASS
    state: READY
        action: Disable(Device)
            remove &VAL_OF_Device from STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES
            remove &VAL_OF_Device from STATUS_MMG_ELTXATLAS_STATUS_FWSETACTIONS
            move_to READY
        action: Enable(Device)
            insert &VAL_OF_Device in STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES
            insert &VAL_OF_Device in STATUS_MMG_ELTXATLAS_STATUS_FWSETACTIONS
            move_to READY

object: STATUS_MMG_ELTXATLAS_STATUS_FWDM is_of_class STATUS_MMG_ELTXATLAS_STATUS_FwDevMode_CLASS
...
object: MMG_SIDE_A::STATUS_MMG_SIDE_A is_of_class ASS_ATLAS_STATUS_CLASS

objectset: STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES is_of_class VOID {MMG_SIDE_A::STATUS_MMG_SIDE_A }
objectset: STATUS_MMG_ELTXATLAS_STATUS_FWSETACTIONS is_of_class VOID {MMG_SIDE_A::STATUS_MMG_SIDE_A }


objectset: STATUS_MMG_ELTXFWCHILDREN_FWSETACTIONS union {STATUS_MMG_ELTXATLAS_STATUS_FWSETACTIONS } is_of_class VOID
objectset: STATUS_MMG_ELTXFWCHILDREN_FWSETSTATES union {STATUS_MMG_ELTXATLAS_STATUS_FWSETSTATES } is_of_class VOID
...
object: MMG_ELTX_FWCNM is_of_class FwChildrenMode_CLASS
...
object: MMG_SIDE_A::MMG_SIDE_A_FWCNM is_of_class ASS_FwChildrenMode_CLASS
...
object: MMG_ELTX_FWM is_of_class FwMode_CLASS
...
object: MMG_SIDE_A::MMG_SIDE_A_FWM is_of_class ASS_FwMode_CLASS

class: MMG_SIDE_A_FwChildMode_CLASS

!panel: FwChildMode.pnl
    state: Excluded	!color: FwStateOKNotPhysics
        action: Include(string OWNER = "",string EXCLUSIVE = "YES")	!visible: 1
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM not_in_state {Excluded, Manual} ) then
            !    else
                    move_to Excluded
            !    endif
            else
                do Include(OWNER=OWNER,EXCLUSIVE=EXCLUSIVE) MMG_SIDE_A::MMG_SIDE_A_FWM
                insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETSTATES
                insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETACTIONS
                insert MMG_SIDE_A::MMG_SIDE_A_FWCNM in FWCHILDRENMODE_FWSETSTATES
            endif
            move_to Included
        action: Manual	!visible: 0
            do Manual MMG_SIDE_A::MMG_SIDE_A_FWM
            insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETSTATES
            remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETACTIONS
            move_to Manual
        action: Ignore	!visible: 0
            do Ignore MMG_SIDE_A::MMG_SIDE_A_FWM
            insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETACTIONS
            remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETSTATES
            move_to Ignored
        action: LockOut	!visible: 1
            move_to LockedOut
        action: Exclude(string OWNER = "")	!visible: 1
            do Exclude(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
            remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETSTATES
            remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETACTIONS
            remove MMG_SIDE_A::MMG_SIDE_A_FWCNM from FWCHILDRENMODE_FWSETSTATES
            move_to Excluded
        ...
        action: Exclude&LockOut(string OWNER = "")	!visible: 0
            move_to LockedOut
    state: Included	!color: FwStateOKPhysics
        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Excluded )  do Exclude
        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Ignored )  move_to IGNORED

        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Manual )  move_to MANUAL

        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Dead )  do Manual

        action: Exclude(string OWNER = "")	!visible: 1
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM not_in_state Included ) then
                if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state InManual ) then
                    do Release(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
                    remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETSTATES
                    remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETACTIONS
                    remove MMG_SIDE_A::MMG_SIDE_A_FWCNM from FWCHILDRENMODE_FWSETSTATES
                else
        ...
        action: Manual(string OWNER = "")	!visible: 1
            do Manual(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
            insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETSTATES
            remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETACTIONS
            move_to Manual
        action: Ignore(string OWNER = "")	!visible: 1
            do Ignore(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
            insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETACTIONS
            remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETSTATES
            move_to Ignored
        action: ExcludeAll(string OWNER = "")	!visible: 1
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM not_in_state {Included,Ignored,Manual} ) then
                if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state InManual ) then
                    do ReleaseAll(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
                    remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETSTATES
                    remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETACTIONS
                    remove MMG_SIDE_A::MMG_SIDE_A_FWCNM from FWCHILDRENMODE_FWSETSTATES
                else
                ...
                do ExcludeAll(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
                remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETSTATES
                remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETACTIONS
                remove MMG_SIDE_A::MMG_SIDE_A_FWCNM from FWCHILDRENMODE_FWSETSTATES
            endif
        ...
        action: Include(string OWNER = "",string EXCLUSIVE = "YES")	!visible: 1
            ...
            move_to Included
        action: Free(string OWNER = "")	!visible: 0
            do Free(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
            move_to Included
        action: SetMode(string OWNER = "",string EXCLUSIVE = "YES")	!visible: 0
            do SetMode(OWNER=OWNER,EXCLUSIVE=EXCLUSIVE) MMG_SIDE_A::MMG_SIDE_A_FWM
        action: ExcludePerm(string OWNER = "")	!visible: 0
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM not_in_state Included ) then
                if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state InManual ) then
                    do Release(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
        ...
        action: Exclude&LockOut(string OWNER = "")	!visible: 1
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM not_in_state Included ) then
                if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state InManual ) then
                    ...
                    if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Dead ) then
                        do Exclude(OWNER=OWNER) MMG_SIDE_A::MMG_SIDE_A_FWM
                        remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETSTATES
                        remove MMG_SIDE_A::MMG_SIDE_A from MMG_ELTX_SIDE_FWSETACTIONS
                        remove MMG_SIDE_A::MMG_SIDE_A_FWCNM from FWCHILDRENMODE_FWSETSTATES
            ...
            move_to LockedOut
    state: Manual	!color: FwStateOKNotPhysics
        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Included )  move_to EXCLUDED
        action: Include(string OWNER = "",string EXCLUSIVE = "YES")	!visible: 1
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Dead ) then
              move_to Manual
            endif
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM not_in_state InManual ) then
              do Include(OWNER=OWNER,EXCLUSIVE=EXCLUSIVE) MMG_SIDE_A::MMG_SIDE_A_FWM
              insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETSTATES
              insert MMG_SIDE_A::MMG_SIDE_A in MMG_ELTX_SIDE_FWSETACTIONS
              insert MMG_SIDE_A::MMG_SIDE_A_FWCNM in FWCHILDRENMODE_FWSETSTATES
            endif
              if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Included ) then
                move_to Included
              endif
            move_to Manual
        action: Exclude(string OWNER = "")	!visible: 1
            ...
            !move_to Manual
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state InManual ) then
                 do SetInLocal MMG_SIDE_A::MMG_SIDE_A_FWM
            endif
            move_to Excluded
        action: Ignore	!visible: 0
            do Ignore MMG_SIDE_A::MMG_SIDE_A_FWM
        ...
        action: Exclude&LockOut(string OWNER = "")	!visible: 1
            ...
            move_to LockedOut
    state: Ignored	!color: FwStateOKNotPhysics
        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Included )  move_to INCLUDED

        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Excluded ) move_to EXCLUDED

        when ( MMG_SIDE_A::MMG_SIDE_A_FWM in_state Dead )  do Exclude
    ...
    state: LockedOut	!color: FwStateOKNotPhysics
        ...
        action: UnLockOut&Include(string OWNER = "",string EXCLUSIVE = "YES")	!visible: 1
            if ( MMG_SIDE_A::MMG_SIDE_A_FWM not_in_state Excluded ) then
            !    else
                    move_to LockedOut
            !    endif
            else
...
object: MMG_SIDE_A_FWM is_of_class MMG_SIDE_A_FwChildMode_CLASS

objectset: FWCHILDMODE_FWSETSTATES is_of_class VOID {MMG_SIDE_A_FWM }
objectset: FWCHILDMODE_FWSETACTIONS is_of_class VOID {MMG_SIDE_A_FWM }
...
objectset: MMG_ELTX_SIDE_FWSETSTATES is_of_class VOID
objectset: MMG_ELTX_SIDE_FWSETACTIONS is_of_class VOID
...
objectset: ATLAS_STATUS_FWSETSTATES is_of_class VOID {STATUS_MMG_ELTX }
objectset: ATLAS_STATUS_FWSETACTIONS is_of_class VOID {STATUS_MMG_ELTX }
...
object: MMG_ELTX:STATUS_MMG_ELTX_FWDM is_of_class FwDevMode_CLASS

object: MMG_ELTX_FWDM is_of_class FwDevMode_CLASS

objectset: FWDEVMODE_FWSETSTATES is_of_class VOID {MMG_ELTX:STATUS_MMG_ELTX_FWDM,
	MMG_ELTX_FWDM }
objectset: FWDEVMODE_FWSETACTIONS is_of_class VOID {MMG_ELTX:STATUS_MMG_ELTX_FWDM,
	MMG_ELTX_FWDM }


objectset: FWCHILDREN_FWSETACTIONS union {MMG_ELTX_SIDE_FWSETACTIONS,
	ATLAS_STATUS_FWSETACTIONS } is_of_class VOID
objectset: FWCHILDREN_FWSETSTATES union {MMG_ELTX_SIDE_FWSETSTATES,
